...
def create_ILP(inst, w, limit):
    ...
    """ Variables """
    x = model.addVars(len(inst.tasks), inst.B, vtype = GRB.BINARY)
    y = model.addVars(len(inst.tasks), inst.B, vtype = GRB.BINARY)
    z00 = model.addVars(len(inst.tasks), inst.B, vtype = GRB.BINARY)
    z10 = model.addVars(len(inst.tasks), inst.B, vtype = GRB.BINARY)
    z01 = model.addVars(len(inst.tasks), inst.B, vtype = GRB.BINARY)
    z11 = model.addVars(len(inst.tasks), inst.B, vtype = GRB.BINARY)

    s = model.addVars(len(inst.tasks), lb = 0, ub = inst.B*inst.T, vtype = GRB.CONTINUOUS)
    c1 = model.addVars(len(inst.tasks), inst.B, vtype = GRB.CONTINUOUS)
    c2 = model.addVars(len(inst.tasks), inst.B, vtype = GRB.CONTINUOUS)
    
    b1 = model.addVars(len(inst.tasks), inst.B, vtype = GRB.CONTINUOUS)
    b2 = model.addVars(len(inst.tasks), inst.B, vtype = GRB.CONTINUOUS)

    r1 = model.addVars(len(inst.tasks), vtype = GRB.CONTINUOUS)
    ...
    for i in range(len(inst.tasks)):
        ...
        for j in range(inst.B):
            ...
            model.addConstr(s[i]+inst.tasks[i][1] >= inst.T*j - M*(1-y[i,j]))
            model.addConstr(s[i]+inst.tasks[i][1] <= inst.T*(j+1) + M*(1-y[i,j]))
            ...
            model.addConstr(b2[i,j] >= s[i] + inst.tasks[i][1] - inst.T*j)
        ...
        model.addConstr(r1[i] >= s[i]+inst.tasks[i][1]- inst.tasks[i][0])

    for j in range(inst.B):
        L = 0
        for i in range(len(inst.tasks)):
            L += inst.tasks[i][1]*z11[i,j] + c1[i,j] + c2[i,j] + inst.T*z00[i,j]
        model.addConstr(r2[j] >= 0)
        model.addConstr(r2[j] >= L - inst.T)
    
    """ Objective """
    model.setObjective(w[0]*sum(r1[i] for i in range(len(inst.tasks))) + w[1]*sum(r2[j] for j in range(inst.B)), GRB.MINIMIZE)
...
class problem():
    def __init__(self, T, B, tasks) -> None:
        self.T = T
        self.B = B
        self.tasks = tasks